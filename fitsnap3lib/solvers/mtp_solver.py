    ...
    def perform_fit(self, a=None, b=None, w=None, fs_dict=None, trainall=False):
        ...
        if pt._rank == 0:
            ...
            if a is None and b is None and w is None:
                ...
                aw, bw = (
                    w[:, np.newaxis] * pt.shared_arrays["a"].array[training],
                    w * pt.shared_arrays["b"].array[training],
                )
            else:
                aw, bw = w[:, np.newaxis] * a[training], w * b[training]

            if (
                "EXTRAS" in self.config.sections
                and self.config.sections["EXTRAS"].apply_transpose
            ):
