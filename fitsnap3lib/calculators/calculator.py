...
class Calculator:
    ...
    def create_a(self):
        ...
        self.number_of_files_per_node = len(
        ...
        if self.config.sections["SOLVER"].solver == "PYTORCH":
            ...
            if self.config.sections["CALCULATOR"].energy:
                ...
                if self.config.sections["CALCULATOR"].per_atom_energy:
                    ...
                    )  # total number of atoms in all configs
                a_len += energy_rows
            ...
            if self.config.args.verbose:
                self.pt.single_print(
                    ">>> Matrix of descriptors and descriptor derivatives takes up ",
                    "{:.4f}".format(
                        100 * a_size / self.config.sections["MEMORY"].memory
                    ),
                    "% of the total memory:",
                    "{:.4f}".format(self.config.sections["MEMORY"].memory * 1e-9),
                    "GB",
                )
            if (
                a_size / self.pt.get_ram() > 0.5
                and not self.config.sections["MEMORY"].override
            ):
                raise MemoryError(
                    "The descriptor matrix is larger than 50% of your RAM. \n Aborting...!"
                )
            elif (
                a_size / self.pt.get_ram() > 0.5
                and self.config.sections["MEMORY"].override
            ):
                self.pt.single_print(
                    "Warning: > 50% RAM. I hope you know what you are doing!"
                )

            self.pt.create_shared_array(
                "a", a_len, a_width, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "b", b_len, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "c", c_len, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "w", b_len, 2, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "t", a_len, 1, tm=self.config.sections["SOLVER"].true_multinode
            )
            if self.config.sections["CALCULATOR"].per_atom_scalar:
                # create per-atom scalar arrays
                self.pt.create_shared_array(
                    "pas", a_len, 1, tm=self.config.sections["SOLVER"].true_multinode
                )
        ...
        # get data arrays for network solvers
            ...
            a_len += self.number_of_atoms  # total number of atoms in all configs
            neighlist_len += self.pt.shared_arrays[
                "number_of_neighs_scrape"
            ].array.sum()
            ...
            # TODO: Pick a method to get RAM accurately (pt.get_ram() seems to get RAM wrong on Blake)
            a_size = (
                neighlist_len * neighlist_width + 2 * c_len * c_width
            ) * self.pt.double_size
            if self.config.args.verbose:
                self.pt.single_print(
                    ">>> Matrix of data takes up ",
                    "{:.4f}".format(
                        100 * a_size / self.config.sections["MEMORY"].memory
                    ),
                    "% of the total memory:",
            ...
            if (
                ...
                raise MemoryError(
                    "The data memory larger than 50% of your RAM. \n Aborting...!"
                )
            elif (
                a_size / self.pt.get_ram() > 0.5
                and self.config.sections["MEMORY"].override
            ...
            self.pt.create_shared_array(
                "x", c_len, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "w", b_len, 2, tm=self.config.sections["SOLVER"].true_multinode
            )
            ...
            self.pt.create_shared_array(
                "positions", a_len, 3, tm=self.config.sections["SOLVER"].true_multinode
            )
            ...
            self.pt.create_shared_array(
                "descriptors",
                neighlist_len,
                self.config.sections["CUSTOM"].num_descriptors,
            )
            ...
            self.shared_index_neighlist = self.pt.fitsnap_dict["sub_neighlist_indices"][
                0
            ]
        ...
        # get data array for MTP solvers

        elif self.config.sections["SOLVER"].solver == "MTPSOLVER":
            a_len = 0  # Per-Atom Scalar Quantities (types)
            b_len = 0  # Per-Configuration Scalar Quantities (energy)
            c_len = 0  # Per-Atom Vector Quantities (forces)
            c_width = 0  # 3 if fitting to forces
            neighlist_len = 0  # number of neighbors for all configs
        ...
        # get data arrays for linear solvers
            ...
            if self.config.args.verbose:
                self.pt.single_print(
                    ">>> Matrix of descriptors takes up ",
                    "{:.4f}".format(
                        100 * a_size / self.config.sections["MEMORY"].memory
                    ),
                    "% of the total memory:",
                    "{:.4f}".format(self.config.sections["MEMORY"].memory * 1e-9),
                    "GB",
                )  # , "on rank", "{:d}".format(self.pt._rank))
            if (
                a_size / self.pt.get_ram() > 0.5
                and not self.config.sections["MEMORY"].override
            ):
            ...
            self.pt.create_shared_array(
                "b", a_len, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.create_shared_array(
                "w", a_len, tm=self.config.sections["SOLVER"].true_multinode
            )
            self.pt.new_slice_a()
            self.shared_index = self.pt.fitsnap_dict["sub_a_indices"][0]
            # pt.slice_array('a')

            self.pt.add_2_fitsnap(
                "Groups", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
            self.pt.add_2_fitsnap(
                "Configs", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
            self.pt.add_2_fitsnap(
                "Row_Type", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
            self.pt.add_2_fitsnap(
                "Atom_I", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
            self.pt.add_2_fitsnap(
                "Testing", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
            self.pt.add_2_fitsnap(
                "Atom_Type", DistributedList(self.pt.fitsnap_dict["sub_a_size"])
            )
        ...
        for key in self.pt.fitsnap_dict.keys():
            if isinstance(self.pt.fitsnap_dict[key], DistributedList):
                ...
                if self.pt.fitsnap_dict[key] is not None and self.pt.stubs != 1:
                    self.pt.fitsnap_dict[key] = [
                        item
                        for sublist in self.pt.fitsnap_dict[key]
                        for item in sublist
                    ]
    ...
    def extras(self):
        ...
        def extras():
            if self.config.sections["EXTRAS"].dump_a:
                np.save(
                    self.config.sections["EXTRAS"].descriptor_file,
                    self.pt.shared_arrays["a"].array,
                )
            if self.config.sections["EXTRAS"].dump_b:
                np.save(
                    self.config.sections["EXTRAS"].truth_file,
                    self.pt.shared_arrays["b"].array,
                )
            if self.config.sections["EXTRAS"].dump_w:
                np.save(
                    self.config.sections["EXTRAS"].weights_file,
            ...
            if self.config.sections["EXTRAS"].dump_dataframe:
                ...
                for key in self.pt.fitsnap_dict.keys():
                    if isinstance(self.pt.fitsnap_dict[key], list) and len(
                        self.pt.fitsnap_dict[key]
                    ) == len(df.index):
