...
class LammpsMtp(LammpsBase):
    ...
    def get_width(self):
        ...
        if self.config.sections["SOLVER"].solver == "MTPSOLVER":
            a_width = 2  # 2 columns for i and j in the sparse neighlist
        else:
            raise NotImplementedError(
                "Only NETWORK solver is implemented with Custom calculator."
            )
    ...
    def _set_box(self):
        ...
        self._set_box_helper(numtypes=self.config.sections["MOMENTTENSOR"].numtypes)

    def _create_atoms(self):
        self._create_atoms_helper(
            type_mapping=self.config.sections["MOMENTTENSOR"].type_mapping
        )
    ...
    def calculate_descriptors(self, x, neighlist, xneigh):
        ...
        rij = np.linalg.norm(diff, axis=1)[
            :, None
        ]  # need for cutoff and various other functions
    ...
    def _collect_lammps_nonlinear(self):
        ...
        num_types = self.config.sections["MOMENTTENSOR"].numtypes
        ...
        assert np.all(
            np.round(self._data["Positions"], decimals=6)
            == np.round(lmp_pos, decimals=6)
        ), "Atom positions inconsistent between read-in data and LAMMPS\nGroup and configuration: {} {}".format(
            self._data["Group"], self._data["File"]
        )
        ...
        for i in range(0, nl.size):
            ...
            if nlist.size > 0:
                for n in np.nditer(nlist):
                    ...
                    transform_x.append(
                        [
                            ptr_pos[n][0] - lmp_pos[tags[n] - 1, 0],
        ...
        neighlist = (
            np.array(neighlist, dtype=int) - 1
        )  # subtract 1 to get indices starting from 0
        ...
        self.pt.shared_arrays["a"].array[
            index : index + num_atoms, 1
        ] = num_neighs_per_atom
        self.pt.shared_arrays["a"].array[
            index : index + num_atoms, 2:
        ] = lmp_pos  # self._data["Positions"]
        index += num_atoms
        ...
        self.pt.shared_arrays["c"].array[index_c : (index_c + (3 * num_atoms))] = (
            self._data["Forces"].ravel() - ref_forces
        )
        self.pt.shared_arrays["x"].array[index_c : (index_c + (3 * num_atoms))] = (
            self._data["Positions"].ravel()
        )
        index_c += 3 * num_atoms
        ...
        self.pt.shared_arrays["neighlist"].array[
            index_neighlist : (index_neighlist + nrows_neighlist)
        ] = neighlist
        self.pt.shared_arrays["xneigh"].array[
            index_neighlist : (index_neighlist + nrows_neighlist)
        ] = xneighs
        self.pt.shared_arrays["transform_x"].array[
            index_neighlist : (index_neighlist + nrows_neighlist)
        ] = transform_x
        ...
        assert np.all(
            np.round(xneighs - lmp_pos[neighlist[:, 1]], 6) == np.round(transform_x, 6)
        )

        # calculate descriptors for standardization

        descriptors = self.calculate_descriptors(
            self._data["Positions"], neighlist, xneighs
        )
        self.pt.shared_arrays["descriptors"].array[
            index_neighlist : (index_neighlist + nrows_neighlist)
        ] = descriptors
        ...
        dindex = dindex + 1
        self.pt.fitsnap_dict["Groups"][self.distributed_index : dindex] = [
            "{}".format(self._data["Group"])
        ]
        self.pt.fitsnap_dict["Configs"][self.distributed_index : dindex] = [
            "{}".format(self._data["File"])
        ]
        self.pt.fitsnap_dict["NumAtoms"][self.distributed_index : dindex] = [
            "{}".format(self._data["NumAtoms"])
        ]
        self.pt.fitsnap_dict["NumNeighs"][self.distributed_index : dindex] = [
            "{}".format(nrows_neighlist)
        ]
        self.pt.fitsnap_dict["Testing"][self.distributed_index : dindex] = [
            bool(self._data["test_bool"])
        ]
    ...
    def _collect_lammps_preprocess(self):
        ...
        lmp_types = self._lmp.numpy.extract_atom_iarray(
            name="type", nelem=num_atoms
        ).ravel()
        lmp_volume = self._lmp.get_thermo("vol")
        ...
        self.pt.shared_arrays["number_of_neighs_scrape"].sliced_array[
            self._i
        ] = number_of_neighs
