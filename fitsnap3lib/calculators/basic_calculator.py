...
from fitsnap3lib.parallel_tools import pt
import numpy as np
    ...
    def convert_to_internal(self):
        ...
        for i, (a_x, a_y, a_z) in enumerate(self._data["Positions"]):
            if i == 0:
                ...
                self._array[i * 3 - 5] = np.arccos(
                    ((this - c) @ (b - c))
                    / (np.linalg.norm(this - c) * np.linalg.norm(b - c))
                )
                self._array[i * 3 - 4] = np.arccos(
                    ((this - b) @ (a - b))
                    / (np.linalg.norm(this - b) * np.linalg.norm(a - b))
                )
                c = b
                b = a
                a = this
